...
async function nextQuestion(goBack, goBackFromResponse, fromDependedOn) {
  ...
  if (currentActiveAnswerType && !isSkipStoreAnswer) {
    if (currentActiveAnswerType == "typeSelection") { 
      await checkAllergie(tempSelectionAns)
    }
...
function closeResponse(fromBack) {
  responseOnGoing = false;
  hasNoResponse = true;
  $("#page5").css("display", "none");
  $("#page4").css("display", "none");
...
async function askQuestion(totalQuizQuestions, counter, fromBack) {
  ...
  if (type == 1) {
    ...
    $("#typeSelection .answerInner").append(`
      <div class="selectionOptions">
        <button data-val="NONE" data-id="NONE" class="selectionBtns selectionBtn none" >None of the above</button>
      </div>
    `);
...
$(document).on("click", ".selectionBtn", function (evt, isCustom) {
  ...
  if ($(this).hasClass("active")) {
    ...
    var noneBtn = document.querySelector(".selectionBtns.selectionBtn.none")

    if (val != "NONE") {
      noneBtn.classList.remove("active");
      noneBtn.classList.remove("highlight");
    }
  }

  if (val == "NONE") {
    tempSelectionAns.splice(0, tempSelectionAns.length);

    var btns = document.getElementsByClassName("selectionBtn");

    for (var i = 0; i < btns.length; i++) {
      var dataVal = btns[i].getAttribute("data-val");
...
$(document).on("click", ".selectionBtns.selectionBtn.none", function handleNoneOfTheAbove() {
  isSkipStoreAnswer = true;
  nextQuestion();
});
...
async function checkAllergie(tempSelectionAns) {
  var sensitiveItems = ["Banana", "Olive", "Sunflowers"];

  var includesAllergies = sensitiveItems.some(item => tempSelectionAns.includes(item));

  if (!includesAllergies) {
    return true;
  }

  var configResponse = await fetch(url_preset + "/admin/terminate-configuration/default");
  var data = await configResponse.json();

  if (data.success) {
    var configData = data.data;

    var message = configData.message;
    var counter = configData.counter;

    var terminatePopup = $('#terminate-quiz');
    var messageElement = $('#terminate-quiz-message');
    var counterElement = $('#terminate-quiz-counter');

    terminatePopup.css('display', 'block');
